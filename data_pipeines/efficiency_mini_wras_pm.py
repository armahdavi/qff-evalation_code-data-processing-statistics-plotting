# -*- coding: utf-8 -*-
"""
Program to calculate the PM efficiencies measured by Mini-WRAS
...
########################################################################
### Step 2: Reading all Mini-WRAS PM data and refine based on Step 1 ###
########################################################################
...
for file in glob.glob('*.xlsx'):
    df = pd.read_excel(file, sheet_name= 'PM values', skiprows = 4)
    ...
    df = df.iloc[:,:4]
    df['PM2.5_10'] = df['PM10'] - df['PM2.5']
    df['PM1_2.5'] = df['PM2.5'] - df['PM1']
    ...
    if j == 1:
        ...
        df_mw_pm = pd.merge(df_mw_pm, df, left_index=True, right_index=True)
...
for i in range(1,4):
    eff = df
    eff['initial_%s' %i] = (1 - df_mw_pm['1_down_mean_%s' %i]/df_mw_pm['1_return_mean_%s' %i]) * 100
    eff['initial_error_%s' %i] = (df_mw_pm['1_down_mean_%s' %i]/df_mw_pm['1_return_mean_%s' %i]) * 100 * np.sqrt((df_mw_pm['1_down_stdev_%s' %i]/df_mw_pm['1_down_mean_%s' %i]).pow(2) + (df_mw_pm['1_return_stdev_%s' %i]/df_mw_pm['1_return_mean_%s' %i]).pow(2))
    
    eff['final_%s' %i] = (1 - df_mw_pm['7_down_mean_%s' %i]/df_mw_pm['7_return_mean_%s' %i]) * 100
    eff['final_error_%s' %i] = df_mw_pm['7_down_mean_%s' %i]/df_mw_pm['7_return_mean_%s' %i] * 100 * np.sqrt((df_mw_pm['7_down_stdev_%s' %i]/df_mw_pm['7_down_mean_%s' %i]).pow(2) + (df_mw_pm['7_return_stdev_%s' %i]/df_mw_pm['7_return_mean_%s' %i]).pow(2))
...
eff.rename(columns = {'index' : 'PM'}, inplace = True)
eff.to_excel(backslash_correct('C:\PhD Research\QFF Evaluation\Processed\mini_wras_eff_pm.xlsx'), index = False)
